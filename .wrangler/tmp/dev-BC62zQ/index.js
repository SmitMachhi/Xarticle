...
var resolveQueryAndBearer = /* @__PURE__ */ __name(async () => {
  const now = Date.now();
  if (state.tweetResultQueryId && state.userTweetsQueryId && state.bearerToken && now - state.queryResolvedAt < QUERY_CACHE_TTL_MS) {
    return {
...
var graphqlFetchUserTweets = /* @__PURE__ */ __name(async (authorId, queryId, bearerToken, guestToken) => {
  const variables = {
    userId: authorId,
  ...
  if (!response.ok) {
    throw new Error(`UserTweets query failed (HTTP ${response.status}).`);
  }
  return await response.json();
}, "graphqlFetchUserTweets");
...
var fetchForwardThreadFromAuthorTimeline = /* @__PURE__ */ __name(async (seedPayload, warnings) => {
  ...
  try {
    const { userTweetsQueryId, bearerToken } = await resolveQueryAndBearer();
    const guestToken = await activateGuestToken(bearerToken);
    const timelineRaw = await graphqlFetchUserTweets(seedMeta.authorId, userTweetsQueryId, bearerToken, guestToken);
    const instructions = timelineRaw?.data?.user?.result?.timeline?.timeline?.instructions || [];
    const { statuses } = parseTimelineEntries(instructions);
    if (!Array.isArray(statuses) || statuses.length === 0) {
      return [];
    }
    const authorStatuses = statuses.filter((node) => {
      const authorId = firstString(node?.core?.user_results?.result?.rest_id);
      return authorId && authorId === seedMeta.authorId;
    });
    const byReplyTo = /* @__PURE__ */ new Map();
    for (const node of authorStatuses) {
      const replyTo = firstString(node?.legacy?.in_reply_to_status_id_str);
      const id = firstString(node?.rest_id, node?.legacy?.id_str);
      if (!replyTo || !id || byReplyTo.has(replyTo)) {
        continue;
      }
    ...
    const seen = /* @__PURE__ */ new Set([seedMeta.statusId]);
    while (descendants.length < FX_THREAD_LIMIT - 1) {
      const nextNode = byReplyTo.get(current);
      const nextId = firstString(nextNode?.rest_id, nextNode?.legacy?.id_str);
...
// .wrangler/tmp/bundle-m7f1Bi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-m7f1Bi/middleware-loader.entry.ts
