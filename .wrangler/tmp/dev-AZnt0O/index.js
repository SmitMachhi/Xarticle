...
var buildGraphqlHeaders = /* @__PURE__ */ __name(async ({
  bearerToken,
  guestToken,
  requestPath,
  includeTransaction,
...
var toArticle = /* @__PURE__ */ __name((tweetNode) => {
  ...
  const blocksFromState = stateBlocks.map((block) => {
  ...
  const blocks = blocksFromState.length > 0 ? blocksFromState : chunkParagraphs(plainText).map((block) => ({ type: "unstyled", text: block.text }));
  return {
...
// .wrangler/tmp/bundle-2dIPRO/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-2dIPRO/middleware-loader.entry.ts
